...
if __name__ == '__main__':
    ...
    for class_folder in os.listdir(args.img_path):
        class_folder_path = os.path.join(args.img_path, class_folder, "camera_00")
        # class_folder_path = os.path.join(args.img_path, class_folder)
        mask_path = os.path.join(args.mask_path, class_folder, 'image.png')
        if os.path.isfile(mask_path):
            mask = cv2.imread(mask_path, -1).astype(bool)
        else:
            mask = mask0

        if os.path.isdir(class_folder_path):
            output_class_folder = os.path.join(args.outdir, 'npy', class_folder)
            os.makedirs(output_class_folder, exist_ok=True)
            output_class_folder_colored = os.path.join(args.outdir, 'colored', class_folder)
            os.makedirs(output_class_folder_colored, exist_ok=True)
            output_class_folder_grey = os.path.join(args.outdir, 'grey', class_folder)
